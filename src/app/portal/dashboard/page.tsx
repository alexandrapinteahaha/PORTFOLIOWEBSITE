...
// Shared card class
const card = "rounded-2xl bg-zinc-50 p-6";
const cardLg = "rounded-2xl bg-zinc-50 p-6 lg:p-8";
...
export default async function PortalDashboardPage() {
  ...
  const [
    ...
    supabase.from("commission_updates")
      ...
      .eq("commission_id", commission.id).eq("is_client_visible", true)
    ...
    supabase.from("commission_payments")
      ...
      .eq("commission_id", commission.id).eq("client_id", client.id),
  ...
  const paidTotal      = (payments ?? []).filter((p) => p.status === "paid").reduce((sum, p) => sum + Number(p.amount_gbp), 0);
  const remaining      = commission.agreed_price_gbp ? Math.max(0, Number(commission.agreed_price_gbp) - paidTotal) : null;
  ...
  const tasks: Task[] = [];
  if (pendingConcept) tasks.push({ title: "Review your concept", desc: "A concept has been prepared for your commission. Please review it and share your thoughts.", href: `/portal/commission/${commission.reference}`, urgent: true });
  ...
  if (pendingPayment && ["final_payment_pending", "final_review"].includes(commission.status)) tasks.push({ title: "Pay remaining balance", desc: `The final payment of ${formatCurrency(Number(pendingPayment.amount_gbp))} is now due.`, paymentId: pendingPayment.id });
  ...
  const calCells  = buildMonthCalendar(calYear, calMonth);
  const bookingUrl = process.env.NEXT_PUBLIC_BOOKING_URL ?? null;

  // Highlight estimated completion if it falls in the current calendar month
  let highlightDay: number | null = null;
  if (commission.estimated_completion) {
    const parts = commission.estimated_completion.split("-").map(Number);
    if (parts[0] === calYear && parts[1] - 1 === calMonth) {
      highlightDay = parts[2];
    }
  }
  ...
  return (
    <div className="container-shell py-10 lg:py-14">
      ...
      <div className="mb-4 grid gap-4 lg:grid-cols-[1fr_272px]">
        ...
        <div className={cardLg}>
          <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
            ...
            <span className="shrink-0 rounded-full border border-ink px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.1em]">
                  ...
                  <div key={step} className="flex flex-1 flex-col items-center">
                    <div className="flex w-full items-center">
                      ...
                      <div className={["flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[10px] font-bold",
                        done ? "bg-ink text-chalk" : active ? "border-2 border-ink bg-zinc-50 text-ink" : "border border-line bg-zinc-50 text-graphite/30",
                      ].join(" ")}>
                        {done ? "✓" : i + 1}
                      </div>
                      {i < PROGRESS_STEPS.length - 1 && <div className={`h-px flex-1 ${i < stageIndex ? "bg-ink" : "bg-line"}`} />}
                    </div>
                    <p className={["mt-2 hidden text-center text-[10px] uppercase tracking-[0.06em] sm:block",
          ...
          <div className="mt-5">
            <Link href={`/portal/commission/${commission.reference}`} className="text-xs text-graphite underline underline-offset-4 hover:text-ink">
        ...
        <div className="flex flex-col rounded-2xl bg-zinc-900 p-5">
          ...
          <div className="mb-4 flex items-center justify-between">
            <h3 className="font-title text-sm font-bold text-white">
              {MONTH_NAMES[calMonth]} <span className="font-normal text-white/40">{calYear}</span>
            </h3>
            <span className="text-[10px] uppercase tracking-[0.1em] text-white/30">Schedule</span>
          </div>

          {/* Day headers */}
          <div className="mb-1 grid grid-cols-7">
            {["M","T","W","T","F","S","S"].map((d, i) => (
              <div key={i} className="flex h-7 items-center justify-center text-[10px] font-semibold text-white/25">{d}</div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7">
            {calCells.map((day, i) => {
              ...
              const isHighlight = day !== null && day === highlightDay;
              return (
                <div
                  key={i}
                  className={[
                    "flex h-7 w-full items-center justify-center rounded-full text-[11px]",
                    !day            ? "" :
                    isToday         ? "bg-white font-bold text-zinc-900" :
                    isHighlight     ? "border border-white/30 font-semibold text-white" :
                                      "text-white/50",
                  ].join(" ")}
                >
                  {day ?? ""}
          ...
          <div className="mt-5 flex-1 border-t border-white/10 pt-5">
            <p className="mb-3 text-[11px] leading-5 text-white/40">Questions about your commission?</p>
            <a
              href={bookingUrl ?? `mailto:hello@alexandrapintea.art?subject=Book a call — ${commission.reference}&body=Hi Alexandra,%0A%0AI'd like to schedule a call regarding my commission (${commission.reference}).%0A%0A`}
              {...(bookingUrl ? { target: "_blank", rel: "noopener noreferrer" } : {})}
              className="block rounded-xl bg-white/10 px-4 py-2.5 text-center text-[11px] font-semibold text-white transition hover:bg-white/20"
            >
              Book a call →
            </a>
            <a
              href={`mailto:hello@alexandrapintea.art?subject=${commission.reference}`}
              className="mt-2 block rounded-xl px-4 py-2.5 text-center text-[11px] font-semibold text-white/40 transition hover:text-white/70"
            >
              Email the studio
            </a>
          </div>
      ...
      <div className="mb-4 grid gap-4 lg:grid-cols-2">
        ...
        <div className={card}>
          ...
          {(payments ?? []).length === 0 && !commission.agreed_price_gbp ? (
            <p className="text-xs leading-6 text-graphite">Payment information will appear here once your commission is confirmed.</p>
          ) : (
            <>
              {commission.agreed_price_gbp && (
                <div className="mb-5 border-b border-line pb-5">
                  <p className="text-xs text-graphite">Commission total</p>
                  <p className="font-title mt-1 text-3xl font-bold">{formatCurrency(Number(commission.agreed_price_gbp), commission.currency)}</p>
                </div>
              )}
              {(payments ?? []).length > 0 && (
              ...
              {remaining !== null && remaining > 0 && (
                <div className="mt-4 border-t border-line pt-4">
                  ...
                  {pendingPayment && <div className="mt-3"><PayButton paymentId={pendingPayment.id} /></div>}
                </div>
              )}
      ...
      <div className="mb-4 grid gap-4 lg:grid-cols-2">
        ...
        <div className={card}>
          <div className="mb-5 flex items-center justify-between">
            ...
            {(documents ?? []).length > 0 && (
              <Link href={`/portal/commission/${commission.reference}`} className="text-xs text-graphite underline underline-offset-4 hover:text-ink">View all</Link>
            )}
          </div>
          {(documents ?? []).length === 0 ? (
        ...
        <div className={card}>
          ...
          {(updates ?? []).length === 0 ? (
            <p className="text-xs leading-6 text-graphite">I&apos;ll post updates here as I work on your artwork — progress notes, photographs and milestones.</p>
          ) : (
            <div className="divide-y divide-line">
              {(updates ?? []).map((update, i) => (
                <article key={update.id} className={i > 0 ? "pt-5" : ""} style={i < (updates ?? []).length - 1 ? { paddingBottom: "1.25rem" } : {}}>
                  ...
                  <p className={`text-xs leading-6 text-graphite line-clamp-4 whitespace-pre-wrap ${update.title ? "mt-1" : ""}`}>{update.body}</p>
                  <Link href={`/portal/commission/${commission.reference}`} className="mt-2 block text-xs text-graphite/60 underline underline-offset-4 hover:text-ink">
                    Read more →
                  </Link>
                </article>
              ))}
      ...
      {otherComms.length > 0 && (
        <div className="mb-4">
          ...
          <div className="grid gap-2">
            {otherComms.map((c) => (
              <Link key={c.id} href={`/portal/commission/${c.reference}`}
                className="flex items-center justify-between rounded-2xl bg-zinc-50 px-5 py-4 transition hover:bg-zinc-100">
                <div>
...
function PayButton({ paymentId, urgent }: { paymentId: string; urgent?: boolean }) {
  return (
      ...
      <button type="submit"
        className={`rounded-xl px-4 py-2 text-xs font-semibold uppercase tracking-[0.1em] transition ${
          urgent
            ...
            : "border border-ink bg-ink text-chalk hover:bg-graphite"
        }`}>
        Pay now →
      </button>
    </form>
